...
import HTNPlanner.PrimitiveTasks.EvaluationFunctions.BuildAndDefendEF;
import HTNPlanner.PrimitiveTasks.EvaluationFunctions.BuildRangedAndDefendEF;
import rts.GameState;
import rts.PhysicalGameState;
import rts.units.Unit;
import util.Helper;

public class p_BuildRangedAndDefendSmall extends PrimitiveTask {
	public p_BuildRangedAndDefendSmall() {
		super();

		this.name = "p_BuildRangedAndDefendSmall";

		this.taskEF = new BuildRangedAndDefendEF();
	}
	...
	public boolean CheckPreconditions(AbstractGameState currentGameState) {
		...
		if (!Helper.PER_BASE_CLOSEST_OPP_BUILDING_INFOS.isEmpty()) {
			...
			if (Helper.DEBUG_PRECONDITIONS) {
				System.out.println(this.name + " does not hold: opp. buildings are reachable");
			}
			return holds;
		}
		...
		if (currentGameState.currentNumBases <= 0 && (currentGameState.currentNumResources < Helper.BASE_TYPE.cost
			...
			holds = false;
			if (Helper.DEBUG_PRECONDITIONS) {
				System.out.println(this.name + " does not hold: no bases/resources available");
			}
		}
	...
	public boolean IsReached(int maxplayer, int minplayer, GameState currentGameState) {
		...
		if (Helper.CURRENT_NUM_BARRACKS >= Helper.DESIRED_NUM_BARRACKS
				&& (Helper.CURRENT_NUM_RANGED >= Helper.DESIRED_NUM_MILITARY
						|| Helper.CURRENT_NUM_RANGED > Helper.CURRENT_NUM_OPP_MILITARY_UNITS)
		 ...
		 * (Helper.CURRENT_NUM_OPP_MOBILE_UNITS-Helper.CURRENT_NUM_OPP_MILITARY_UNITS)/2
		 * )) { reached = true; }
		 */
		/*
