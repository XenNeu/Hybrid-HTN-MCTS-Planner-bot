...
public class p_BuildAndDefendBig extends PrimitiveTask {
	public p_BuildAndDefendBig() {
		super();

		this.name = "p_BuildAndDefendBig";
	...
	public boolean CheckPreconditions(AbstractGameState currentGameState) {
		...
		if (currentGameState.currentNumBases > 0 && !Helper.PER_BASE_CLOSEST_OPP_INFOS.isEmpty()) {
			for (Long baseID : Helper.PER_BASE_CLOSEST_OPP_INFOS.keySet()) {
				...
				if ( (Helper.MAX_MAP_DIST > 64 && dist < (float) (Helper.MAX_MAP_DIST / 6))
						|| Helper.MAX_MAP_DIST <= 64 && dist < (float) (Helper.MAX_MAP_DIST / 4)) {
					holds = false;
					if (Helper.DEBUG_PRECONDITIONS) {
	...
	public boolean IsReached(int maxplayer, int minplayer, GameState currentGameState) {
		...
		if (Helper.CURRENT_NUM_BARRACKS >= 1
				&& (Helper.CURRENT_NUM_MILITARY >= Helper.DESIRED_NUM_MILITARY
						|| Helper.CURRENT_NUM_MILITARY > Helper.CURRENT_NUM_OPP_MILITARY_UNITS + 3)
				&& (Helper.ANY_OPP_BUILDING_REACHABLE || Helper.ANY_OPP_UNIT_REACHABLE))
		{
			reached = true;
		}

		if (Helper.CURRENT_NUM_MILITARY >= 3 && Helper.MAX_MAP_DIST == 64
				&& (Helper.ANY_OPP_BUILDING_REACHABLE || Helper.ANY_OPP_UNIT_REACHABLE))
		{
		...
		if (Helper.CURRENT_NUM_MILITARY - Helper.CURRENT_NUM_OPP_MILITARY_UNITS >=3 
